...
//                判断当前日期是否已预约
                long conditionByOrderDate = orderSettingDao.findConditionByOrderDate(orderSetting.getOrderDate());
                if(conditionByOrderDate>0){
//                    已经进行了预约设置，执行更新操作
...
//        根据日期查询是否已经进行了预约设置
        Long count=orderSettingDao.findConditionByOrderDate(orderDate);
        if(count>0){
//            当前日期已经进行了预约设置，需要执行更新操作
        orderSettingDao.editNumberByOrderDate(orderSetting);
        }else{
//            当前日期没有执行预约设置，需要执行插入操作
            orderSettingDao.add(orderSetting);
