package com.non7.order.service.impl;



import com.non7.client.member.MemberClient;
import com.non7.order.dao.OrderDao;
import com.non7.order.dao.OrderSettingDao;
import com.non7.order.service.OrderService;
...
//        3、检查用户是否重复预约（同一个用户在同一天预约了同一个套餐），如果是重复预约则复发完成再次预约
        ...
        Member member = memberClient.findByTelephone(telephone);
        if(member!=null){
//            判断是否在重复预约
...
//            根据条件进行查询
            ...
            memberClient.add(member);//自动完成会员注册
        }
        //        5、预约成功，更新当日的已预约人数
